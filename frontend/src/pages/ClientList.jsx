...
export default function ClientList() {
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      const msg = err.response?.data?.message || 'Erro ao desativar cliente';
  ...
  const filteredClients = safeClients.filter(c => {
    ...
    const matchStatus =
      statusView === 'todos' ||
      (statusView === 'ativos' && c?.status !== 'Inativo') ||
      (statusView === 'inativos' && c?.status === 'Inativo');
    return matchSearch && matchStatus;
  });

  const inactiveCount = safeClients.filter(c => c?.status === 'Inativo').length;
  const activeCount = safeClients.length - inactiveCount;
  ...
  return (
    <div className="animate-fade">
      ...
      <div className="glass-panel search-panel" style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '6px', padding: '4px', background: 'rgba(255,255,255,0.04)', border: '1px solid var(--border-glass)', borderRadius: '10px' }}>
          {[
            { id: 'ativos', label: `Ativos (${activeCount})` },
            { id: 'todos', label: `Todos (${safeClients.length})` },
            { id: 'inativos', label: `Inativos (${inactiveCount})` }
          ].map(option => (
            <button
              key={option.id}
              type="button"
              onClick={() => setStatusView(option.id)}
              style={{
                height: '34px',
                padding: '0 0.85rem',
                border: 'none',
                borderRadius: '8px',
                cursor: 'pointer',
                background: statusView === option.id ? 'rgba(212,175,55,0.16)' : 'transparent',
                color: statusView === option.id ? 'var(--primary-gold)' : 'var(--text-light)',
                fontSize: '0.75rem',
                fontWeight: 800
              }}
            >
          ...
          filteredClients.map((client) => (
            ...
            >
              <div className="client-card-body" onClick={() => navigate(`/clientes/${client.id}/inventario`)}>
                        ...
                        >
                          ...
                          {isAdmin && (
                            <>
                              ...
                              >
                                <Trash2 size={15} /> Desativar Cliente
                              </button>
                            </>
                          )}
                        </div>
                ...
                <p style={{ fontSize: '0.72rem', fontWeight: 700, color: 'rgba(255,255,255,0.38)', margin: '0 0 0.7rem' }}>
                  {Number(client?.equipment_count || 0)} equipamento(s) vinculado(s)
                </p>
      ...
      {/* Modal de confirmacao de desativacao */}
      {confirmDelete && (
        <div
        ...
        >
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '1.5rem' }}>
              ...
              <div>
                <h3 style={{ color: '#fff', fontWeight: 800, fontSize: '1.1rem', margin: 0 }}>Confirmar Desativacao</h3>
                <p style={{ color: 'var(--text-light)', fontSize: '0.8rem', margin: 0 }}>O registro sera preservado no historico</p>
              </div>
            </div>
            <p style={{ color: 'var(--text-light)', fontSize: '0.9rem', lineHeight: 1.6, marginBottom: '2rem' }}>
              Voce esta prestes a desativar o cliente <strong style={{ color: '#fff' }}>{confirmDelete.name}</strong>. Ele podera ser reativado depois, e os equipamentos vinculados continuam preservados.
            </p>
