...
export default function UsersPage() {
  ...
  const permissionProfiles = [
    {
      role: 'admin',
      title: 'Administracao completa',
      items: ['Gerencia usuarios', 'Remove documentos e artigos', 'Acesso total aos modulos']
    },
    {
      role: 'goldtech_team',
      title: 'Operacao tecnica',
      items: ['Cria e edita documentacao', 'Cria e edita conhecimento', 'Sem gestao de usuarios']
    },
    {
      role: 'user',
      title: 'Operador basico',
      items: ['Acesso ao inventario operacional', 'Sem documentacao interna', 'Sem gestao de usuarios']
    }
  ];
  ...
  /* ── Editar ── */
  const openEdit = (user) => {
    setFormEdit({ name: user.name, email: user.email, role: normalizeRole(user.role) });
    setModalEdit(user);
  };
  ...
  const RoleBadge = ({ role }) => (
    <span style={{
      ...
      background: normalizeRole(role) === 'admin' ? 'rgba(212,175,55,0.15)' : 'rgba(255,255,255,0.06)',
      color: normalizeRole(role) === 'admin' ? 'var(--primary-gold)' : 'var(--text-light)',
      border: `1px solid ${normalizeRole(role) === 'admin' ? 'rgba(212,175,55,0.3)' : 'var(--border-glass)'}`,
    }}>
      {normalizeRole(role) === 'admin' ? <ShieldCheck size={11} /> : <User size={11} />}
      {getRoleLabel(role)}
    </span>
  );
    ...
    <div className="animate-fade">
      ...
      <div style={{
        ...
        {permissionProfiles.map(profile => (
          <div key={profile.role} className="glass-panel" style={{ padding: '1.25rem' }}>
            <RoleBadge role={profile.role} />
            <h3 style={{ color: '#fff', fontSize: '0.95rem', margin: '0.85rem 0 0.65rem', fontWeight: 800 }}>{profile.title}</h3>
            <div style={{ display: 'grid', gap: '0.45rem' }}>
              {profile.items.map(item => (
                <p key={item} style={{ margin: 0, color: 'var(--text-light)', fontSize: '0.78rem', display: 'flex', alignItems: 'center', gap: '0.45rem' }}>
                  <CheckCircle size={13} color="var(--accent-emerald)" /> {item}
                </p>
              ))}
            </div>
          </div>
          ...
          <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <select className="form-control-premium" value={formCreate.role} onChange={e => setFormCreate({ ...formCreate, role: e.target.value })}>
                ...
                <option value="goldtech_team">Tecnico GoldTech</option>
